...
CREATE POLICY "Users can view registrations by invite code" ON public.registrations
  FOR SELECT USING (
    public.is_super_admin() OR
    (approved_company_id IS NOT NULL AND public.is_company_admin(approved_company_id)) OR
    (invite_code IS NOT NULL AND invite_code = current_setting('request.headers', true)::json->>'invite_code')
  );
